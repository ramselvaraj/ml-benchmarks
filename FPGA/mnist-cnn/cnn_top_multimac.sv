...
module cnn_top_multimac #(
    parameter int    PAR_MAC = 32,

    parameter string W1_FILE = "w1.mem",   // K*K*IMG_C*C1_OUT = 288,   int8
    parameter string B1_FILE = "b1.mem",   // C1_OUT            = 32,    int32
    parameter string W2_FILE = "w2.mem",   // K*K*C1_OUT*C2_OUT = 18432, int8
    parameter string B2_FILE = "b2.mem",   // C2_OUT            = 64,    int32
    parameter string W3_FILE = "w3.mem",   // K*K*C2_OUT*C3_OUT = 36864, int8
    parameter string B3_FILE = "b3.mem",   // C3_OUT            = 64,    int32
    parameter string W4_FILE = "w4.mem",   // FLAT_LEN*FC1_OUT  = 36864, int8
    parameter string B4_FILE = "b4.mem",   // FC1_OUT           = 64,    int32
    parameter string W5_FILE = "w5.mem",   // FC1_OUT*FC2_OUT   = 640,   int8
    parameter string B5_FILE = "b5.mem"    // FC2_OUT           = 10,    int32
)(
    input  logic clk,
    input  logic rst,
    input  logic start,
  ...
  localparam int DOT1 = K*K*IMG_C;       // 9    — conv1 kernel volume
  localparam int DOT2 = K*K*C1_OUT;      // 288  — conv2 kernel volume
  localparam int DOT3 = K*K*C2_OUT;      // 576  — conv3 kernel volume
  localparam int DOT4 = FLAT_LEN;        // 576  — fc1 input length
  localparam int DOT5 = FC1_OUT;         // 64   — fc2 input length
  ...
  logic signed [7:0]  img  [0:IMG_H*IMG_W-1];         // 784

  logic signed [31:0] c1   [0:C1_H*C1_W*C1_OUT-1];   // 26*26*32  = 21632
  logic signed [7:0]  p1   [0:P1_H*P1_W*C1_OUT-1];   // 13*13*32  =  5408

  logic signed [31:0] c2   [0:C2_H*C2_W*C2_OUT-1];   // 11*11*64  =  7744
  logic signed [7:0]  p2   [0:P2_H*P2_W*C2_OUT-1];   //  5* 5*64  =  1600

  logic signed [31:0] c3   [0:C3_H*C3_W*C3_OUT-1];   //  3* 3*64  =   576
  logic signed [7:0]  c3q  [0:FLAT_LEN-1];            //              576

  logic signed [31:0] fc1  [0:FC1_OUT-1];             // 64
  logic signed [7:0]  fc1q [0:FC1_OUT-1];             // 64
  logic signed [31:0] fc2  [0:FC2_OUT-1];             // 10
  ...
  // P1 pool — operates on c1 (C1_H × C1_W × C1_OUT)
  ...
  // P2 pool — operates on c2 (C2_H × C2_W × C2_OUT)
  ...
  function automatic signed [7:0] requant8(input signed [31:0] v);
    ...
    begin
      sh = v >>> 8;
      if      (sh >  127) requant8 =  8'sd127;
      else if (sh < -128) requant8 = -8'sd128;
      else                requant8 =  sh[7:0];
    end
  endfunction
  ...
  function automatic signed [31:0] conv1_sum(
      input int base, input int y, input int x, input int outc);
    int m, kh, kw, ic;
    logic signed [31:0] s;
    begin
      s = 0;
  ...
  function automatic signed [31:0] conv2_sum(
      input int base, input int y, input int x, input int outc);
    int m, kh, kw, ic;
    logic signed [31:0] s;
    begin
  ...
  function automatic signed [31:0] conv3_sum(
      input int base, input int y, input int x, input int outc);
    int m, kh, kw, ic;
    logic signed [31:0] s;
    begin
