...
def estimate_svm_ops(n_samples, n_features, n_support):
    ops_per_sample = float(n_support) * float(n_features) * 2.0
    return ops_per_sample * float(n_samples)


def estimate_svm_arith_intensity(n_samples, n_features, n_support):
    bytes_per_sample = (float(n_features) + float(n_support) * float(n_features) + float(n_support)) * 4.0
    total_bytes = bytes_per_sample * float(n_samples)
    total_ops = estimate_svm_ops(n_samples, n_features, n_support)
...
n_support = svm.support_.shape[0]
n_features = X_test_scaled.shape[1]
ops_total = estimate_svm_ops(len(X_test), n_features, n_support)
...
arith_intensity = estimate_svm_arith_intensity(len(X_test), n_features, n_support)
