...
constexpr int SEQ_LEN       = 128;     // Fixed at synth time. Runtime-variable
                                        // seq_len would hurt Fmax; for sweep
                                        // data, re-synthesize at each S.
constexpr int HEAD_DIM      = 64;      // All our models use D=128.
constexpr int NUM_Q_HEADS   = 1;       // H_q
constexpr int NUM_KV_HEADS  = 1;       // H_kv  (MHA=32, GQA=8, MQA=1)
...
constexpr int BQ = 8;      // Query rows per tile.
constexpr int BK = 128;      // Key/Value rows per tile.
