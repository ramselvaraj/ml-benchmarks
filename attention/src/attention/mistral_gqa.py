"""Mistral-7B attention layer (GQA, 32 Q heads / 8 KV heads, RoPE).

Caveat: Mistral-7B-v0.1 uses a 4096-token sliding-window attention
pattern. This implementation runs full causal masking regardless (same
as Llama-2 for seq_len <= 4096); beyond 4096 tokens the numerics will
diverge from Mistral's true forward. For the variant comparison
(MHA vs. MQA vs. GQA kernel cost) that's fine — SWA is a Mistral-
specific accuracy choice, not a per-variant kernel property — but
timing at seq_len > 4096 should be read as "dense GQA at this shape,"
not "Mistral's deployed attention."

The checkpoint key naming matches Llama's.
"""
...
class MistralGQA(AttentionModule):
    HF_KEYS = {
    ...
    def load_weights(self, path: str) -> None:
        ...
        if missing_expected:
            raise RuntimeError(
                f"Missing expected keys when loading Mistral GQA from {path}: "
